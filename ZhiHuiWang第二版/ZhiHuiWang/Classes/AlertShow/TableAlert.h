...
typedef UITableViewCell* (^MLTableAlertTableCellsBlock)(TableAlert *alert, NSIndexPath *indexPath);
typedef void (^MLTableAlertRowSelectionBlock)(NSIndexPath *selectedIndex);
typedef void (^MLTableAlertCompletionBlock)(void);


@interface TableAlert : UIView <UITableViewDataSource, UITableViewDelegate>
